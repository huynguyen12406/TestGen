...
public class BackendService {
    ...
    private void loadApiKeys() {
        ...
        // Fallback to default encoded keys (for easy setup - users can clone and run
        // immediately)
        if (apiKey == null || apiKey.isEmpty()) {
    ...
    public String extractTextFromImage(File imageFile, Consumer<String> logger)
        ...
        try {
            ...
            if (!ocrService.isTesseractAvailable()) {
                ...
                throw new Exception(
                        "Tesseract not found. Please download from https://github.com/UB-Mannheim/tesseract/wiki and extract to tesseract/ folder.");
            }
            ...
            if (errorMsg.contains("Tesseract not available")) {
                ...
                logger.accept(
                        "[ERR] Tải Tesseract: https://github.com/UB-Mannheim/tesseract/wiki → giải nén vào tesseract/");
                throw new Exception(
                        "Tesseract not available. Download from https://github.com/UB-Mannheim/tesseract/wiki and extract to tesseract/ folder.",
                        e);
            } else if (errorMsg.contains("timeout")) {
    ...
    /**
     * Sinh test cases với VERIFICATION - expected output được tạo bằng cách chạy AC
     * code.
     * Đây là workflow mới để fix bug: AC code bị đánh giá WA do AI tính sai
     * expected output.
     * 
     * Workflow:
     * 1. Generate AC code
    ...
    public List<TestCase> generateTestcasesWithVerification(Problem problem, int count,
        ...
        try {
            ...
            List<com.competitive.ai.ResponseParser.TestInput> testInputs = aiService.generateTestInputsOnly(huyProblem,
                    count);
            ...
            for (int i = 0; i < testInputs.size(); i++) {
                ...
                com.competitive.service.CompilerService.RunResult runResult = compiler.run(compileResult,
                        testInput.getInput(), timeLimitMs);
        ...
        switch (lang.toUpperCase()) {
            ...
            default:
                throw new IllegalArgumentException(
                        "Unsupported language: " + lang + ". Only Java and C++ are supported.");
        }

        // Use try-with-resources to ensure cleanup
        try (com.competitive.service.CompilerService compiler = new com.competitive.service.CompilerService()) {

            logger.accept("[RUN] Biên dịch code " + lang + "...");

            // Compile the code
            com.competitive.service.CompilerService.CompileResult compileResult = compiler.compile(code, compilerLang,
                    className);
